...
export const searchResume = async (resume: expanded_resume, query: string) => {
    ...
    const response = await openai.chat.completions.create({
        ...
        messages: [
            ...
            {
                role: 'user',
                content: `Answer the following question using the provided context. Return the text in markdown format use your best judgement on presentation. Lean towards lists then tables then paragraphs. 
